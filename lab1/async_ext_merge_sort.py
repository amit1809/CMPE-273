...
OUTPUT_SORTED_FILE = "output/async_sorted.txt"

async def sort(arr, filename):
    """
    ...
    if len(arr) > 1:
        ...
        await sort(L, filename)  # Sorting the first half
        await sort(R, filename)  # Sorting the second half
...
async def mergeSortedToFile(arr):
    """
    ...
    for entry in entries:
        ...
        with open("output/Temp/input/"+entry) as file:
            for line in file:
                line = int(line.strip())
                arr.append(line)
        await mergeSortedToFile(arr)

async def sortFile(filename):
    inpList = []
    with open(filename) as file:
        for line in file:
            line = int(line.strip())
            inpList.append(line)
    await sort(inpList, filename)
